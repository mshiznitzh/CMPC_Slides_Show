...
def main():
    ...
    currentyeardreleasedf=Project_Data_df[(Project_Data_df['Estimated_In-Service_Date'].dt.year == date.today().year) &
    ...
    currentyeartotalproject=sum([len(currentyeardreleasedf.index), len(currentyeardInServicedf.index), len(currentyeardInScopingdf.index),
                                            len(currentyeardOnHolddf.index)])
    print('')
    print(''.join([str(currentyeartotalproject), strlist[4], str(date.today().year), strlist[5]]))

    bigprojects=len(currentyeardreleasedf[currentyeardreleasedf['Approved_WA_Amount'] >= 200000]) + len(currentyeardInServicedf[currentyeardInServicedf['Approved_WA_Amount'] >= 200000])

    bigprojectsda=len(currentyeardreleasedf[(currentyeardreleasedf['Approved_WA_Amount'] >= 200000) &
                    (currentyeardreleasedf['BUDGETITEMNUMBER'] == '00003407')]) + len(currentyeardInServicedf[(currentyeardInServicedf['Approved_WA_Amount'] >= 200000) &
    ...
    print(' '.join([str(bigprojectsnotdainservice), strlist[11], str(bigprojectsnotdareleased + bigprojectsnotdainservice)
                       , strlist[12]]))
    ...
    print(strlist[0])
    print(' '.join([str(len(HVbreakerreplacementscurrentyeardf[HVbreakerreplacementscurrentyeardf['PROJECTSTATUS'] == 'Released'])), strlist[3]]))
    print(' '.join([str(
        len(HVbreakerreplacementscurrentyeardf[HVbreakerreplacementscurrentyeardf['PROJECTSTATUS'] == 'In-Service'])),
                    strlist[4]]))
    print(' '.join([str(
        len(HVbreakerreplacementscurrentyeardf[HVbreakerreplacementscurrentyeardf['PROJECTSTATUS'] == 'No Capital Spend'])),
                    strlist[5]]))
    print('')
    print(strlist[1])
    print(' '.join([str(
